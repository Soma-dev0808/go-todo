...
func TestUpdate(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "異常系_必須項目がなかった場合バリデーションエラーになること（task）",
			requestPathParam: handler.UpdateRequestPathParam{ID: 1},
			requestBodyParam: handler.UpdateRequestBodyParam{
				Status: model.Created,
			},
			want_status_code: http.StatusBadRequest,
...
func TestDelete(t *testing.T) {
	...
	tests := []struct {
	...
	} {
		{
			...
			request: handler.DeleteRequestParam{ID: 1},
			usecase: &mockTodo{
				mockDelete: func() error {
					return nil
				},
...
func TestFind(t *testing.T) {
	...
	tests := []struct {
	...
	} {
		{
			...
			request: handler.FindRequestParam{ID:1},
			usecase: &mockTodo{
				mockFind: func() (*model.Todo, error) {
					return &expected, nil
				},
		...
		},
		{
			...
			usecase: &mockTodo{
				mockFind: func() (*model.Todo, error) {
					return nil, errors.New("xxx error")
				},
			},
			want_status_code: http.StatusInternalServerError,
		},
